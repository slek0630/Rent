...
@interface RSuperViewController ()

@property (nonatomic, strong) UIPanGestureRecognizer *popRecognizer;

@end

@implementation RSuperViewController
...
- (void)handlePopRecognizer:(UIScreenEdgePanGestureRecognizer*)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.interactivePopTransition = [[RCommonVcTransition alloc] init];
        if ([self.navigationController respondsToSelector:@selector(RpopViewControllerAnimated:)]) {
            [self.navigationController performSelector:@selector(RpopViewControllerAnimated:) withObject:[NSNumber numberWithBool:YES]];
        } else {

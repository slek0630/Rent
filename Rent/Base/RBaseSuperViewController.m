...
@interface RBaseSuperViewController ()
...
@implementation RBaseSuperViewController
...
- (void)viewDidLoad {
    ...
    //ios7.0以上的系统都要改变下位置
    BOOL isNeedOffset = [RCommonUtils isUpperSDK];
    for (UIView *subview in [self.view subviews]) {
        if ([self.titleNavBar isEqual:subview]) {
            [self.view bringSubviewToFront:self.titleNavBar];
            if (isNeedOffset) {
                [RUIUtils updateFrameWithView:subview superView:self.view isAddHeight:YES];
            }
        }else{
            BOOL isChange = YES;
            if (isNeedOffset) {
                isChange = [RUIUtils updateFrameWithView:subview superView:self.view isAddHeight:NO];
            }else{
    ...
    //normal title view
    if ([_titleNavBar isMemberOfClass:[RTitleNavBarView class]]) {
...
-(void) initTitleNavBar
{
    ...
    _titleNavBar = [[RTitleNavBarView alloc] init:self];
    
    if ([_titleNavBar isMemberOfClass:[RTitleNavBarView class]]) {
        _titleNavBarLeftButton = ((RTitleNavBarView *) _titleNavBar).toolBarLeftButton;
        _titleNavBarLeftButton2 = ((RTitleNavBarView *) _titleNavBar).toolBarLeftButton2;
        _titleNavBarRightBtn = ((RTitleNavBarView *) _titleNavBar).toolBarRightButton;
        _titleNavBarRightBtn2 = ((RTitleNavBarView *) _titleNavBar).toolBarRightButton2;
        _segmentedControl = ((RTitleNavBarView *) _titleNavBar).segmentedControl;
        _titleNavImageView = ((RTitleNavBarView *) _titleNavBar).navImageView;
...
-(void) setTitle:(NSString *) title
{
    if (!_titleLabel) {
        if ([_titleNavBar isMemberOfClass:[RTitleNavBarView class]]) {
            _titleLabel = [((RTitleNavBarView *) _titleNavBar) setTitle:title];
        }else{
            super.title = title;
        }
    }else{
...
-(void) setTitle:(NSString *) title font:(UIFont *) font
{
    if (!_titleLabel) {
        if ([_titleNavBar isMemberOfClass:[RTitleNavBarView class]]) {
            _titleLabel = [((RTitleNavBarView *) _titleNavBar) setTitle:title font:font];
        }
    }else{
        [_titleLabel setText:title];
...
-(void) setContentInsetForScrollView:(UIScrollView *) scrollview
{
    ...
    CGFloat topInset = 0;
    if (!self.titleNavBar) {
        topInset = R_Default_TitleNavBar_Height;
        if ([RCommonUtils isUpperSDK]) {

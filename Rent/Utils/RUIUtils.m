...
{
    RAlertView *alert = [[RAlertView alloc] initWithTitle:title message:msg cancelButtonTitle:@"确定"];
    [alert show];
}
...
{
    return @"无法访问你的相机。\n请到手机系统的[设置]->[隐私]->[相机]允许租房使用相机";
}

+ (NSString*)documentOfLocationDenied {
    return @"无法获取你的位置信息。\n请到手机系统的[设置]->[隐私]->[定位服务]中打开定位服务，并允许租房使用定位服务";
}
+ (NSString*)documentOfAssetsLibraryDenied {
    NSString *errorString = nil;
    double version = [[[UIDevice currentDevice] systemVersion] doubleValue];
    if (version > 6.0) {
        errorString = @"租房没有权限访问您的相册,请在【隐私】【照片】中允许【租房】访问";
    }else{
        errorString = @"租房没有权限访问您的相册,请打开您的【定位服务】";
    }
    return errorString;
}
...
+ (UIImage*)addOperationAsset:(ALAsset *) asset
{
    ...
    if (fullResolutionImageSize.height/fullResolutionImageSize.width > [UIScreen mainScreen].bounds.size.height/[UIScreen mainScreen].bounds.size.width) {
        ...
        if (MIN(image.size.width, image.size.height) > LS_MAX_IMAGE_WIDTH) {
            ...
            //                    }
            //                }
            image = [RUIUtils scaleImage:image toSize:newSize];
